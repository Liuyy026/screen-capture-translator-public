...
def translate(page, model, progress, save, block_id=None, image=None):
    if not page["blocks"]:
        return
    audit_cache(page)

    def save_with_overlays():
        """Persist translations together with geometry refreshed for them."""
        # The overlay signature includes translation readiness because
        # uncertain OCR-only blocks can become safe local rectangles after
        # the model returns text.
        try:
            if image is not None:
                _overlay.ensure_page_overlays(page, image=image)
        except Exception as exc:
            # Keep the translated text and force the reader to retry geometry.
            page.pop("overlay_signature", None)
            print(f"Overlay refresh failed: {exc}", file=sys.stderr)
        save(page)
    # A page containing only preserved/hand-edited lines needs no model
    # lookup.  This also makes ``skip`` a true translation-task exclusion.
    eligible = [b for b in page["blocks"] if not b.get("edited") and
                b.get("overlay") != "skip" and
                (block_id is None or b["id"] == block_id)]
    if not eligible:
        return
    ...
    if page.get("model") != model:
        ...
        page["model"] = model
        save_with_overlays()
    targets = [b for b in page["blocks"] if not b.get("edited") and b.get("overlay") != "skip" and
               (block_id is None or b["id"] == block_id) and
    ...
    for start in range(0, len(targets), 8):
        ...
        try:
            ...
            translated, errors = parse_translations(
                response["message"]["content"], {b["id"] for b in batch},
                {b["id"]: b.get("source", "") for b in batch})
            for b in batch:
